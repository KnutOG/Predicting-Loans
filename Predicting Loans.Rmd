...
```{r}
logit.model <- glm(default ~ ., data = listings, family = "binomial")
summary(logit.model)

# Predicting the probability of an employee leaving using the Logit model
listings_logit <- NULL
listings_logit$default <- predict(logit.model, newdata = listings, type = "response")

# Converting probabilities to a binary outcome (1 for Prob > 0.5, otherwise 0)
listings_logit$default[listings_logit$default > 0.5] <- 1
listings_logit$default[listings_logit$default != 1] <- 0

# Creating a cross table to verify the accuracy of the Logit model
CrossTable(x = listings_logit$default, y = listings[26834:33542, 11], prop.chisq = FALSE)
```
