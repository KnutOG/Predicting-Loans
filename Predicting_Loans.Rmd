...
```{r}
#listingsample$is_homeowner <- as.factor(listingssample$is_homeowner)
...
listingByIncomeRangeDesc <-listings %>%
    group_by(income_range_description) %>%
    summarise(sum = n(), prop = sum(loan_status == 1)/n())




plot<-ggplot(data=listingByIncomeRangeDesc, aes(x=income_range_description, y=prop, fill = (income_range_description)))+
  geom_bar(stat="identity" )
plot 
...
#drop aggregates
listingsample<- listingsample[, !(colnames(listingsample) %in% c("scorex", "dti_wprosper_loan"))]
